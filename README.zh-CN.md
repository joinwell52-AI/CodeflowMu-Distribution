...
> [!NOTE]
> 下一候选 `V2.2.1 Preview 12` 已在本地完成构建、隔离安装和启动验收，新增“手动选择安装目录”和“按完整发行版本号发现、确认、下载并安装完整升级包”。它尚未上传，因此本页仍只把 Preview 11 列为可下载版本；在 Preview 12 正式发布前，不把这些新增能力反向承诺给 Preview 11。
...
从 `V2.2.1 Preview 12` 起，CodeFlowMu 的 Windows 发行版采用完整安装包升级，不要求客户每次回到下载页重新找文件：
